...
def attention(query, key, value, mask=None, dropout=None, group_prob=None, no_cuda=False):
    ...
    if group_prob is not None:
        ...
        p_attn = p_attn*group_prob.unsqueeze(1) # Product attention
    else:
        p_attn = F.softmax(scores, dim = -1)
    if dropout is not None:
        p_attn = dropout(p_attn)
    return torch.matmul(p_attn, value), p_attn # self-attention and Product attention
...
class GroupAttention(nn.Module):
    ...
    def forward(self, context, eos_mask, prior):
        ...
        neibor_attn = torch.sqrt(neibor_attn*neibor_attn.transpose(-2,-1) + 1e-9) #  √ (p(i,i+1) × p(i+1,i)) neighbor attention
        neibor_attn = prior + (1. - prior)*neibor_attn # Hierarchical Constraint (Constituent attention)

        t = torch.log(neibor_attn + 1e-9).masked_fill(a==0, 0).matmul(tri_matrix) # log Constituent attention
        g_attn = tri_matrix.matmul(t).exp().masked_fill((tri_matrix.int()-b)==0, 0) # exp log Constituent attention
