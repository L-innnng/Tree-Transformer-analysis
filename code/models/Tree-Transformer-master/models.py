...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, mask):
        ...
        for layer in self.layers:
            att, x, group_prob,break_prob = layer(x, mask,group_prob)
            break_probs.append(break_prob)
            atts.append(att)
        x = self.norm(x)
        break_probs = torch.stack(break_probs, dim=1)
        return self.proj(x), atts, break_probs
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask, group_prob):
        ...
        att = self.self_attn.get_attention()

        return att, self.sublayer[1](x, self.feed_forward), group_prob, break_prob