...
def transform_split(path, binarized,dummy_tokens, root_label,encode_unary_leaf):
    ...
    #print(sequences,sequences_for_leaf_unaries)
    ...
    with codecs.open(path_dest,"w",encoding="utf-8") as f:
        for sentence in sequences:
            ...
            for word, postag,gold in sentence:
                f.write(u"\t".join([word,str(postag),str(gold)])+u"\n") #str
            f.write(u"\n")    
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test", dest="test", help="Path to the parenthesized test file",default='data/PP_ambiguity/ground_trees.txt')
    parser.add_argument("--output", dest="output", 
                        help="Path to the output directory to store the dataset", default='data/PP_ambiguity')
    parser.add_argument("--encode_unaries", action="store_true", dest="encode_unaries", help="Activate this option to encode the leaf unary chains as a part of the label", default=True)
    parser.add_argument("--os", action="store_true",help="Activate this option to add both a dummy beggining- (-BOS-) and an end-of-sentence (-EOS-) token to every sentence", default=True)
    ...
    write_linearized_trees("/".join([args.output,"test."+ext]), test_sequences)
    
    if not args.encode_unaries:
        write_linearized_trees("/".join([args.output, "test."+ext_unary]), test_leaf_unary_chains)
